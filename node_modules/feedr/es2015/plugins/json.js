...
module.exports.parse = function parseJSON(_ref, next) {
	...
	// Detect
	var isJSON = feed.parse === 'json' || ['.json', '.jsonp', '.js'].indexOf(feed.extension) !== -1 || response.headers['content-type'].indexOf('javascript') !== -1 || response.headers['content-type'].indexOf('json') !== -1;
	if (!isJSON) {
		next();
		return;
	}
